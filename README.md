...
> **Agent-Orchestrated AI Product**  
> 이 프로젝트는 Vibe Coding + 멀티 에이전트 실행 체계를 실제 제품 개발 루프에 적용한 사례입니다.
...
또한 계획(Planner) → 구현(Executor) → 점검(Reviewer) 루프를 기반으로, 여러 에이전트를 병렬로 활용해 개발 속도와 품질을 동시에 확보하는 방식을 채택했습니다.

## 🤖 이 프로젝트가 보여주는 점 (AI Orchestration)

- 🗂️ 카드 생성 오케스트레이션: `quick` / `deep` 모드 분기
- 🔎 리서치 오케스트레이션: OpenAI/External 어댑터 + fallback(stub) 체인
- 🎨 이미지 오케스트레이션: `openai`, `gemini`(=`nano-banana` 별칭 포함) 라우팅 + fallback
- ⚙️ 비동기 잡 오케스트레이션: 연구 작업 생성/상태/결과/취소 API + running future 정리
...
1. Planner 에이전트가 변경 범위/리스크/검증 기준을 정의  
2. Executor 에이전트들이 서버/웹/테스트 작업을 병렬 분할 실행  
3. Reviewer 성격 에이전트로 API 계약/호환성/누수 위험을 재점검  
4. 최종적으로 `test + ktlint + build`를 통과한 변경만 반영

위 루프를 반복하며 "빠른 실험"과 "운영 품질" 사이 균형을 맞추는 것이 Cardra 개발 방식의 핵심입니다.

## 🧩 AI 도구 역할 분담

- `Codex`: 구현 중심 실행자. 멀티 파일 변경, 테스트/빌드 검증, 자동화 루프 실행
- `Claude Code`: 설계/리뷰 보조. 요구사항 정리, 리스크 점검, 문서/계획 품질 강화
- `Gemini CLI`: 리서치/아이디어 확장 보조. 대안 탐색, 빠른 비교 검토, 보완 조사

핵심은 단일 도구 의존이 아니라, **역할 기반 분업 오케스트레이션**으로 개발 리드타임과 완성도를 동시에 확보하는 것입니다.

## 📚 AI 운영 문서 체계

- `AGENTS.md`: 디렉터리별 개발 규칙, 품질 게이트, 실행 정책 정의
- `SKILL.md` 계열 문서: 에이전트 작업 절차/트리거/검증 체크리스트 표준화
- `.claude`, `.codex`, `.gemini`, `.agent-policies`: 도구별 지침과 라우팅 정책 관리

즉, "AI에게 일을 시킨다"가 아니라 **문서화된 운영 규칙 위에서 AI 협업을 관리**하는 방식을 적용했습니다.
...
⚠️ 주의: `--madmax`는 승인/샌드박스 우회 모드이므로 신뢰 환경에서만 사용하세요.
