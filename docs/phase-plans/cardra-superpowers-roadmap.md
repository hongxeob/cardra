...
[완료] `FallbackAgentAdapter` 적용: 위임 실패 시 fallback card 3종 반환
...
- **Task 2.1**: 에이전트 구현체 분리
  - `MockAgentAdapter`와 별도 `FallbackAgentAdapter`를 둬 실패 시 기본 템플릿으로 전환

[진행] 실제 외부 API(원격) 어댑터는 다음 단계에서 연결 예정
- **Task 2.1-a**: `ExternalAgentAdapter` 인터페이스/클라이언트 클래스 추가(계획)
...
2. API 테스트 추가 ✅ (GET 카드 조회 포함)
3. AgentAdapter 예외 처리/타임아웃 정책 문서화 ✅
4. 문서 업데이트 후 팀 공유
5. Fallback 동작 테스트 추가 ✅
