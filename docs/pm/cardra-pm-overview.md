...
## 바로 보이는 현재 구조
- Controller: `apps/server/src/main/kotlin/com/cardra/server/api/{Card,Health,Research,Recommend}Controller.kt`
- Service: `apps/server/src/main/kotlin/com/cardra/server/service/{Card,agent,research,recommend}`
- DTO: `apps/server/src/main/kotlin/com/cardra/server/dto/{Research,Recommend}Dtos.kt`
...
- [진행 필요] 이미지 생성 실제 연동(현재는 프리셋 URL 중심의 placeholder 스타일)
...
3. 추천 스택을 `user_event_history` + 후보 생성기 + fallback 엔진으로 2단계 분리
4. 에러 스키마/리트라이 표준을 Health/API 응답 규약 문서에 묶음
5. 카드 응답의 렌더 메타와 실제 디자인 규칙 매핑(variant/style/media/cta/tags) 정합성 점검
6. 이미지 생성 pipeline PoC(placeholder 기반 → 생성 모델 기반) 도입
