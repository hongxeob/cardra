    ...
    override fun composeCards(keyword: String): List<CardItem> {
        ...
        return listOf(
            CardItem(
                ...
                body =
                    "${refs[0]} 기준으로 보면 현재 이슈는 단발성 반응보다 누적된 시장 변수와 " +
                        "정책·공급·수요 신호가 동시에 작동하는 구간입니다. 특히 24~72시간 이슈를 " +
            ...
            CardItem(
                ...
                body =
                    "${refs[1]} 쪽은 가격 신호·정책 변동·플랫폼 노출 지표가 동시 변동합니다. " +
                        "단기 리스크와 중기 확장성을 분리해 점검하면 오판을 줄일 수 있고, " +
            ...
            CardItem(
                title = "$keyword 핵심 요약",
                body =
                    "최종 판단은 출처 2곳 이상 교차 확인 루틴이 가장 안전합니다. 수치 갱신이 " +
                        "발생할 때는 타임스탬프를 함께 보존하고, 다음 리스크 점검에서 동일 지표를 " +
